...
from .attention import SCALayer
...
class PSConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, parts=4, bias=False):
        ...
        self.mask = torch.zeros(self.conv.weight.shape).byte().cuda().bool()
...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
                 last_level, head_conv, out_channel=0, attention='channel_conv1d_add', pyconv=True):
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                modules = []
                if 'hm' in head and pyconv:
                    modules.append(PSConv2d(channels[self.first_level], head_conv, kernel_size=3, padding=1, bias=True))
                    print('===> use PSConv2d')
                else:
                    modules.append(nn.Conv2d(channels[self.first_level], head_conv,
                                             kernel_size=3, padding=1, bias=True))
                modules.append(nn.ReLU(inplace=True))
                # add attention GC_block
                if 'hm' in head and attention is not None:
                    # modules.append(nn.Conv2d(head_conv, head_conv, kernel_size=1, stride=1, padding=0, bias=True))
                    # modules.append(nn.ReLU(inplace=True))
                    # modules.append(ContextBlock2d(head_conv, head_conv // 4, 'att', ['channel_add']))
                    modules.append(SCALayer(head_conv, fusion=attention))

                modules.append(nn.Conv2d(head_conv, classes, kernel_size=1, stride=1, padding=0, bias=True))
                ...
                #     nn.ReLU(inplace=True),
                #     # add attention GC_block
                #     # ContextBlock2d(head_conv, head_conv // 4, 'att', ['channel_add']) if 'hm' in head else None,
                #
...
def get_pose_net_py(num_layers, heads, head_conv=256, down_ratio=4, attention='channel_conv1d_add', pyconv=True):
